...
const GenerateImages = () => {
  ...
  const imageStyle =  ['Realistic','Ghibli Style', 'Anime Style', 'Cartoon Style',
    'Fantasy Style', 'Realistic Style','3D Style' , 'Portrait style', 'Sceneries Style'
    ];
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        <textarea
          ...
          className="w-full p-2 px-3 mt-2 outline-gray-800 text-sm rounded-md border border-gray-800"
              ...
              <label className='relative cursor-pointer'>
                ...
                checked = {publish} className='sr-only peer'/>
                  <div className='w-9 h-5 bg-slate-300 rounded-full
                   peer-checked:bg-purple-800 transition'></div>

                   <span className='absolute left-1 top-1 w-3 h-3 bg-white
                   rounded-full transition-transform duration-300 transform peer-checked:translate-x-4'></span>
              </label>
        ...
        gap-2 bg-gradient-to-r from-[#C341F6] to-[#8E37EB] text-white
        px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer"
        >  {
          loading ? <span className='w-4 h-4 my-1 rounded-full
            boarder-2 border-t-transparent animate-spin'></span> 
          :  <Image className="w-5" />
        }
      ...
      >
        <div className="flex items-center gap-3">
          <Image className="w-6 h-5 text-[#8E37EB]" />
